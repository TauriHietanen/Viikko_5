...
<title>Asiakastietojen listaus</title>
</head>
<body>
	<table id="listaus">
		<thead>	
...
function poista(asiakas_id) {
	if(confirm("Poista asiakastiedot " + asiakas_id +"?")){
		$.ajax({url:"asiakkaat/"+asiakas_id, type:"DELETE", dataType:"json", success:function(result) { //result on joko {"response:1"} tai {"response:0"}
	        if(result.response==0){
	        	$("#ilmo").html("Asiakkaan tietojen poisto epäonnistui.");
	        }else if(result.response==1){
	        	$("#rivi_"+asiakas_id).css("background-color", "red"); //Värjätään poistetun asiakkaan rivi
	        	alert("Asiakkaan " + asiakas_id +" tietojen poisto onnistui.");
				haeAsiakkaat();        	
			}
	    }});
}
